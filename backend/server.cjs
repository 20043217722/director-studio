/**
 * Director Studio — API Proxy Server v2.1
...
const INVITE_TTL_MIN = 10 // 邀请码有效期：10分钟
const SMS_TTL_MS = 5 * 60 * 1000 // 短信验证码：5分钟
const SMS_COOLDOWN_MS = 60 * 1000 // 短信重发间隔：60秒
...
function db(name) {
  ...
  catch { return { invitation_codes: [], users: [], visits: [], sms_codes: {} }[name] || [] }
}
function save(name, data) { fs.writeFileSync(path.join(DATA_DIR, name + '.json'), JSON.stringify(data, null, 2)) }
...
function smsCode() { return String(100000 + Math.floor(Math.random() * 900000)) }
...
async function authRoutes(req, res, pn, method) {
  ...
  if (pn === '/api/auth/generate' && method === 'POST') {
    ...
    if (sha256(req.headers['x-admin-auth']||'') !== ADMIN_HASH && (req.headers['x-admin-auth']||'') !== 'admin123')
      return j(res, 403, { error: '管理员密码错误' })
    const n = Math.min(b.count || 1, 50) // 最多50个
    const codes = db('invitation_codes')
    const list = []
    for (let i = 0; i < n; i++) {
      const c = { code: randStr(8), created_at: iso(), expires_at: new Date(Date.now() + INVITE_TTL_MIN*60*1000).toISOString(), is_used: false, used_by: null, used_at: null }
      codes.push(c); list.push(c)
    }
    save('invitation_codes', codes)
    hlog('auth', `生成 ${n} 个邀请码`)
    return j(res, 200, { codes: list, active: codes.filter(c => !c.is_used && new Date(c.expires_at) > new Date()).length })
  }

  // ─── GET /api/auth/codes — Admin: 查看所有邀请码 ───
  ...
  if (pn === '/api/auth/send-sms' && method === 'POST') {
    ...
    save('sms_codes', sms)
    hlog('sms', `[→] ${phone} = ${code}`)
    return j(res, 200, { ok: true, msg: '验证码已发送', dev: code }) // 正式上线删 dev 字段
  }
  ...
  if (pn === '/api/auth/login' && method === 'POST') {
    ...
    if (se.tries >= 5) { delete sms[phone]; save('sms_codes', sms); return j(res, 429, { error: '尝试次数过多，请重新获取' }) }
    if (Date.now() > se.exp) { delete sms[phone]; save('sms_codes', sms); return j(res, 400, { error: '验证码已过期，请重新获取' }) }
    se.tries++; save('sms_codes', sms)
    ...
    // Clean SMS
    delete sms[phone]; save('sms_codes', sms)
  ...
  // ─── POST /api/auth/check-invite — 预检邀请码（不消耗） ───
...
const server = http.createServer(async (req, res) => {
  ...
  // Health
  if (pn === '/health') {
    return j(res, 200, { status: 'ok', configured: Object.entries(KEYS).filter(([,v])=>v).map(([k])=>k), features: ['auth','invites','tracking'] })
  }
...
server.listen(PORT, () => {
  ...
  hlog('info', `Director Studio v2.1 :${PORT}`)
  hlog('info', `Keys: ${cfg.length?cfg.join(','):'NONE'}`)
  hlog('info', `Auth: SMS login | Invite: 8-char/${INVITE_TTL_MIN}min TTL | Admin: admin123`)
})
