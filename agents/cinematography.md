...
### 四、画质约束（根据用户参考图/描述自动匹配风格画像）

> **🎯 风格自动匹配规则: 从视觉解析师的影调参数 + 用户描述中提取风格关键词，自动匹配以下画像。如用户提供参考图→以图中影调为准；如用户仅描述想法→从描述中推断最接近的风格。**

#### 按景别的负面约束（优先于风格约束）

MCU/CU/ECU·近景/特写:
  面部结构不变形·保留毛孔+细纹+微汗+伤疤+皮肤纹理·无塑料CG肤质·微表情克制不夸张
  五官比例跨帧稳定·手指关节自然不粘连·发丝睫毛细节锐利·手持物不可变形·手势端庄

WS/EWS·全景/远景:
  空间结构不漂移·材质纹理真实无贴图感·光影方向与色温一致·色彩不溢出
  不做自动HDR提亮·大气透视自然衰减(远处微雾·近处清晰)·雕花纹理不可丢失·云纹立体非平面

动作帧·静态定格:
  运动残影保留(滞后≤0.2s)·碎片/粒子保留飞行轨迹·服装和发丝的物理惯性保留
  不完全静止·保留微弱的物理残余动能·能量特效保留渐变层次

#### 通用约束（根据参考图/描述·动态生成）

> **🔍 自动生成规则: 从视觉解析师的影调分析 + 用户描述 + 画面主体类型中提取关键约束点，按以下优先级生成通用约束：**
> 1. **画面主体类型**: 人物为主→面部/肢体优先 | 场景为主→空间/材质优先 | 道具为主→细节/质感优先
> 2. **参考图影调**: 若视觉解析师已输出光比/阴影类型/主光方向→约束中保留光影一致性
> 3. **构图类型**: 对称→居中 | 纵深→透视准确 | 负空间→留白比例 | 动态→运动模糊保留
> 4. **用户描述关键词**: 提取"压抑""温暖""冷峻""快速"等情绪/动作词→转化为视觉约束

**通用约束模板（AI根据上述规则填充）：**

避免: [结构错乱·光影矛盾·透视错误·空间漂移·贴图感平面材质]

按主体类型:
  [人物为主]→面部不变形·皮肤保留毛孔+细纹+微汗+伤疤·五官跨帧稳定·手指关节自然·比例不失调
  [场景为主]→空间结构不漂移·材质纹理真实·光影方向与色温一致·大气透视自然·色彩不溢出
  [道具/器物为主]→物体不变形·保留真实材质纹理(磨损·包浆·裂纹)·表面反射自然·细节锐利

按参考图/视觉解析师输出:
  [光影一致性]→光比·阴影类型·主光方向与参考图一致·不可被AI统一或自动优化
  [色温一致性]→暖色系/冷色系/双色温·按参考图保留·不可被AI统一为单色温
  [风格标签一致性]→从视觉解析师提取的风格标签·注入约束(如"冷蓝灰低饱·中调高反差硬")

按用户描述关键词:
  [从用户输入提取]→如"压抑"→暗部不可被HDR提亮·"温暖"→保留暖色调·"快速"→保留运动模糊和残影·"干净"→材质不可过度老化·"神圣"→保留辉光和柔焦


#### 风格画像自动匹配（按用户输入/参考图选择以下一个或多个组合）

#### 风格画像自动匹配（按用户输入/参考图选择以下一个或多个组合）

[🎬 真人写实/电影级]:
  避免: 塑料CG肤质(保留毛孔·疤痕·细纹·微汗)·过度美颜·色彩溢出·过度HDR·CG光滑表面取代真实肌理
  双色温被统一·自然材质(木纹·石材·织物·金属·皮肤)失去真实纹理·暗部死黑无细节
  适用场景: 历史剧·社会写实·纪录片·商业摄影·人物肖像

[🎨 三维渲染二维·Arcane式/双城之战式]:
  避免: 手绘笔触被平滑成光滑表面·轮廓线被抗锯齿抹除·PBR物理材质替换手绘高光和排线
  照片级贴图覆盖手绘肌理·物理精确光线追踪取代绘画式光影·暗部被AI自动补全破坏层次
  60fps过度流畅插值·角色比例被AI向写实矫正·背景被过度细化失去绘画感
  适用场景: Arcane风格·三渲二动画·手绘厚涂·油画质感3D

[🕸️ 风格化动画·Spider-Verse式]:
  避免: 半调网点被抗锯齿抹除·漫画分色被转为连续色调·Ben-Day dots被当噪点消除
  印刷纹理被数字平滑·平面空间层次被3D景深取代·AI补帧到流畅(保留抽帧感)
  速度线被运动模糊替代·漫画夸张比例被AI向写实矫正·CMYK分色被转为RGB全色谱
  适用场景: 蜘蛛侠风格·漫画动画·波普艺术·平面设计感动画

[🎞️ 复古胶片/16mm/8mm]:
  避免: 胶片颗粒被数字降噪抹除·胶片光晕(halation)被去除·扫描线被修复
  胶片色彩偏移被矫正到中性·抖动被数字稳定消除·胶片特有的暖黄肤色偏移被矫正
  暗部胶片灰蓝底调被数字提亮·跳帧和光斑被AI修复·色域被扩大到数字色域
  适用场景: 年代戏·怀旧·独立电影·纪录片·实验影像

[🏯 东方古典/国风水墨]:
  避免: 西方油画式的浓重阴影·过度写实的物理光照·现代材质的建筑和道具出现
  水墨笔触被数字平滑掉·留白被AI自动填补·传统纹样(云纹/回纹/龙纹)被简化或变形
  人物服饰出现现代剪裁·建筑出现西方柱式·色彩过饱和失去古画褪色感
  适用场景: 古装·仙侠·武侠·历史剧·神话·国风动画·水墨风格

[🌆 赛博朋克/科幻]:
  避免: 霓虹光效过度漫散(保留锐利边缘)·全息投影无扫描线(保留隔行扫描感)
  金属表面被统一光滑处理(保留磨损和工业感)·雾霾被完全清除(保留空气密度)
  巨型建筑无尺度感·义体改造失去机械细节·城市灯光过暖失去冷峻感
  适用场景: 赛博朋克·科幻·未来都市·机甲·太空站·反乌托邦

[🏜️ 废土/末日/后启示录]:
  避免: 材质过于干净无老化·色彩过于鲜艳·天空过蓝·建筑结构完整无破损
  金属无锈蚀·织物无破损·地面无裂缝·空气过于清澈·植被过于茂盛
  适用场景: 废土·末日·灾后·荒野·生存·沙漠朋克

[⛩️ 日式赛璐璐/Anime/新海诚式]:
  避免: 轮廓线被抗锯齿抹除·色彩渐变过于平滑失去赛璐璐分层感·光线追踪取代手绘光影
  角色比例被AI向真人矫正·背景过度细化失去动画感·天空和云的层次被简化
  适用场景: 日式动画·赛璐璐风格·新海诚式·吉卜力式·轻小说插画风

[🎪 皮克斯/迪士尼3D CGI]:
  避免: 材质过于写实失去卡通感·角色比例被AI向真人矫正·光影过于复杂失去动画感
  材质反射过于物理精确(保留卡通式的简化高光)·背景失去插画感
  适用场景: 3D动画·皮克斯式·迪士尼式·卡通渲染·家庭向

[◼️ 黑白胶片/Noir/黑色电影]:
  避免: 彩色元素意外出现·对比度过低·暗部被数字提亮·胶片颗粒被降噪抹除
  阴影层次被HDR压缩·高光溢出失去细节·暖调被数字校正为中性灰
  适用场景: 黑色电影·惊悚·悬疑·年代戏·艺术摄影·极简

[🎮 虚幻引擎/实时渲染/游戏引擎]:
  避免: 光线追踪噪点·LOD切换可见·贴图mipmap模糊·阴影贴图分辨率不足
  抗锯齿伪影·屏幕空间反射瑕疵·植被/头发穿插·布料解算错误
  适用场景: 游戏过场·实时渲染·虚拟制片·预可视化

[⚡ 故障艺术/Glitch/Vaporwave]:
  避免: 画面过于干净无噪点·色彩过于自然·无RGB分离·无扫描线·无信号干扰效果
  画面过于稳定(保留抖动和撕裂感)·色彩被校正为自然色调
  适用场景: 实验影像·音乐视频·赛博朋克·复古未来主义·数字艺术

[🖼️ 油画/水彩/手绘质感]:
  避免: 笔触被平滑消失·画布纹理被覆盖·颜料厚度被压平·色彩过于数字均匀
  水彩的渗透和晕染被数字模拟取代·油画的厚涂笔触被抹平
  适用场景: 艺术动画·独立短片·风格化·诗影像·美术设定

#### 通用摄影质感约束(所有风格可选加)
避免: [数字防抖过度(保留手持呼吸感)·AI补帧·自动慢动作插值·过度锐化·镜头像差被数字矫正]
避免: [数字色彩校正到完美中性(保留胶片微偏/风格化色偏)·JPG压缩伪影·暗部噪点被降噪抹除]

[用户自定义约束]: [根据用户的具体要求补充·如"画面左侧必须保留30%留白"·"色调必须与参考图一致"]
---
---
