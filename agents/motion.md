...
### 一、设备选择
1、摄影机: [从参考库选型·必填型号] | T[T值] | 快门角度[°] | ISO[值] | 机位(距主体[m]·[平/俯/仰]·[角度]°) | 镜头[球面/变形·焦段mm·型号] | 滤镜[类型·强度] | 画面质感([胶片型号]·色彩科学) | 画幅[比] | 分辨率 | 帧率[fps] | 风格[真人写实/2D动漫/3D皮克斯/韩漫国漫/赛博朋克] | 视觉锚点: [画面最具辨识度的视觉元素·Seedance将其作为跨帧风格锁定的参照物]
...
1、（起始s — 结束s）:
   ...
   画面景别: [景别类型]
   ── 画面内容 ──
   前景: [距镜头最近的物体/人物·虚化程度·占画面比例]
   主体: [角色名·3-5个外貌锚点·站位·占画面比例]
   陪体: [后方人物/物体·距主体距离·朝向·视线/动作关系]
   背景: [场景环境·地点·时间·3个关键视觉元素·色值]
   ── 人物情绪 ──
   面部表情: [眉头·眼神·嘴角·微表情变化轨迹·从X情绪→Y情绪的过渡]
   肢体动作: [姿态·手势·重心转移·运动部位·速度·节奏感·从A→B的轨迹]
   说话语气: [音量·语速·音色·情绪色彩·台词原文/内心独白]
   ── 人物站位与空间关系 ──
   空间模式: [从摄影指导经典站位库选择: 过肩/背对背/并列平行/高低位差/门框分隔/边缘站位/中心孤立/低角度仰拍/高角度俯拍/背影站位]
   空间叙事: [这个站位在讲什么权力关系/情感——1句话]   ── 色彩标注 ──
   [颜色名=HEX]


2、（起始s — 结束s）:
   运镜方式: [同上格式]
   ...（每个时间段按同样子字段展开，编号递增，逐秒推进）

   ── 共享负面约束(全镜统一·不逐段重复) ──
   按画面风格选择下方对应画像·选取5-8条最关键约束即可·不必全部复制
   [见下方「风格化负面约束画像」章节·根据当前影片风格选择对应画像填入此处]

### 四、画面光影+色彩基调
1、整体情绪氛围 | 色彩基调(主色=色名=HEX·60% + 辅色=色名=HEX·30% + 强调=色名=HEX·10%) | 影调色调: [色系色温饱和｜调性反差光质·光位] | 光源类型 | 色温[K] | 光源方向[°]·高度[°] | 风格类型(真人/2D/动漫/平面/赛博) | LUT/胶片参考 | 饱和度

### 四·五、色彩锁定(Seedance专属·防漂移)
   关键色值: [色名=HEX·锁定原因] — [色名=HEX·锁定原因] — [色名=HEX·锁定原因]
   锁定策略: [在每个时序段的色彩标注中强制重复标注以上色值·利用Seedance的色值重复锚定机制抑制跨帧漂移]
   漂移容差: ΔE ≤ [值]（超出此范围判定为色彩漂移废片·需重新生成）
   Seedance色偏规律: 暖色系(红/橙/褐)易向品红漂移 | 冷色系(蓝/灰)易向青绿漂移 | 中性灰易向紫蓝漂移

### 五、声音+音效设计
1、环境底噪(类型·dB·混响类型·混响时间s·声场定位) | 关键音效(起始s—结束s·材质特征·dB·声场定位·画面位置) | 背景音乐(出入点·风格·BPM·Key·dB·声场定位)
\`\`
\`\`\`
